              ...
              var PanelContainer = MainHDiv.Helpers.DivC("col-md-12 p-n-lr");
              {
                  var HomeContainer = PanelContainer.Helpers.DivC("tabs-container");
                  {
                      var AssessmentsTab = HomeContainer.Helpers.TabControl();
                      {
                          ...
                          var HomeContainerTab = AssessmentsTab.AddTab("Movie Table");
                          {
                          ...
                          var HomeContainerTab2 = AssessmentsTab.AddTab("Snacks Table");
                          {
                              var Row2 = HomeContainerTab2.Helpers.DivC("row margin0");
                              {
                                  var RowCol2 = Row2.Helpers.DivC("col-md-12");
                                  {
                                      //RowCol2.Helpers.HTML().Heading2("Editable Table");
                                      //RowCol2.Helpers.HTMLTag("p").HTML = "Create a basic table by generating your classes using the CSLA Extension.";
                                      //RowCol2.Helpers.HTML("<p>You can alter the stored procedure to retrieve the required information (e.g. Genre Name/Description instead of the foreign key/id).</p>");
                                      //RowCol2.Helpers.HTML("<p>Should you have a dropdown data annotation on one of the properties on your object, the table will automatically change the field to a drop down list. Changing the TableFor properties allows for adding new items/removing of items from your object list. All that is left to do is to save the list.</p><br>");
                                      //RowCol2.Helpers.HTML("<p>In order to Add, Update or Delete items from the list, write a JavaScript function that in return calls a web callable method to save the list on the ViewModel.</p>");
                                      //RowCol2.Helpers.HTML("<pre><code>var SaveResult = MovieList.TrySave();</code></pre><br>");
                                      //RowCol2.Helpers.HTML().Heading3("Example");


                                      var SnackList = RowCol2.Helpers.TableFor<MELib.Snacks.Snack>((c) => c.SnackList, true, true);
                                      {
                                          ...
                                          var SnackListRow = SnackList.FirstRow;
                                          {
                                              ...
                                              {
                                                  SnackTitle.Style.Width = "300px";
                                              }
                                              var SnackTypeID = SnackListRow.AddColumn(c => c.SnackTypeID);
                                              {
                                                  SnackTypeID.Style.Width = "200px";
                                              }
                                      ...
                                      var SaveList2 = RowCol2.Helpers.DivC("col-md-12 text-right");
                                      {
                                          var SaveBtn2 = SaveList2.Helpers.Button("Save", Singular.Web.ButtonMainStyle.NoStyle, Singular.Web.ButtonSize.Normal, Singular.Web.FontAwesomeIcon.None);
                                          {
                                              SaveBtn2.AddClass("btn-primary btn btn btn-primary");
                                              SaveBtn2.AddBinding(Singular.Web.KnockoutBindingString.click, "SaveSnacks($data)");
                                          }
                                      }
  ...
  <script type="text/javascript">
    ...
    var SaveMovies = function (obj) {
      ...
      var SaveSnacks = function (obj) {
          ViewModel.CallServerMethod('SaveSnackList', { SnackList: ViewModel.SnackList.Serialise(), ShowLoadingBar: true }, function (result) {
              if (result.Success) {
                  MEHelpers.Notification("Successfully Saved", 'center', 'success', 3000);
              }
              else {
